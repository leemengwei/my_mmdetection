...
# lmw data pipeline for show
@PIPELINES.register_module()
class GlanceOnData(object):
    ...
    def __call__(self, results):
        data = results['img']
        if len(data.shape) == 3:
            self.plt.imshow(np.abs(data))
            self.plt.draw()
            self.plt.pause(self.wait_sec)
            self.plt.close()
        else:
            print("***Warning img data shape not (3)", data.shape, data.sum(), results)
            pass
        return results
